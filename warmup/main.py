...
break_b = pygame.image.load("./Assets/break.png")
...
def handel_movment(keys_pressed, last_direction, real_direction):
	# also can add big char
	if keys_pressed[pygame.K_d] and real_direction != 3:
		last_direction = 1
	elif keys_pressed[pygame.K_s] and real_direction != 4:
		last_direction = 2
	elif keys_pressed[pygame.K_a] and real_direction != 1:
		last_direction = 3
	elif keys_pressed[pygame.K_w] and real_direction != 2:
		last_direction = 4
...
def draw_game_win(head, body, apple, score, best_score_saved, walls):
	win.fill(WITHE)
	win.blit(top_1, (0, 0))
	win.blit(top_2, (0, 93))
	...
	for i in walls:
		win.blit(break_b, (i[0]-23, i[1]-23))
...
def new_apple(game_map, body, head, walls): # walls,
...
def main():
	...
	while game:
		...
		walls = [[25, 375], [75, 375], [125, 375], [325, 275], [325, 325], [325, 375], [325, 425], [325, 475]]
		...
		for x in range(12):
		    for y in range(12):
		        if 0 < 50*x-25 < 500 and 100 < 50*y+75 < 600:
		            continue
		        border.append([50*x-25, 50*y+75])

		head = [75, 125]
		body = [[25, 125]]
		head_c = head.copy()
		apple = new_apple(game_map, body, [head], walls)
		add_to_end = False
		real_direction = 1
		user_direction = 1
		...
		while run:
			...
			user_direction = handel_movment(keys_pressed, user_direction, real_direction)
			...
			if fps_level == 20:
				...
				if head_c in border or head_c in body or head_c in walls:
					run = False
					break
				...
				if head_c == apple[0]:
					...
					if len(body) + 2 == 100:
						...
						state = 1
						break
					apple = new_apple(game_map, body, [head, head_c], walls)
			...
			draw_game_win(head, body, apple, score, best_score_saved, walls)
